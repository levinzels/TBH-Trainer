...
bool c_widgets::text_field(std::string name, std::string icon, char* buf, int buf_size) {
    ...
    text_field_state* animstate = gui->anim_container(&animstate, window->GetID((std::stringstream{} << id << "text_field_animstate").str().data()));

    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImVec2(gui->content_avail().x, SCALE(25));
    ImRect total = ImRect(pos, pos + size);

    gui->item_size(total); gui->item_add(total, id);

    bool user_clicked = total.Contains(GetMousePos()) && gui->mouse_clicked(0) && IsWindowHovered();
    gui->easing(animstate->text_clr, animstate->active ? clr->accent.Value : clr->text_inactive.Value, 10.f, dynamic_easing);
    gui->easing(animstate->alpha, animstate->active ? 1.f : 0.f, 6.f, static_easing);

    draw->rect_filled(window->DrawList, total.Min, total.Max, draw->get_clr(clr->widget_layout), SCALE(2));

    if (strlen(buf) == 0) {
        draw->text_clipped(window->DrawList, font->get(inter_medium, 9), total.Min + SCALE(10, 0), total.Max, draw->get_clr(clr->text_inactive), name.data(), gui->text_end(name.data()), 0, { 0, 0.5 });
    }

    draw->text_clipped(window->DrawList, font->get(icons, 10), total.Min, total.Max - SCALE(10, 0), draw->get_clr(clr->accent), icon.data(), 0, 0, { 1, 0.5 });

    gui->push_var(style_var_frame_rounding, 0);
    gui->push_font(font->get(inter_medium, 9));

    bool ret = text_field_ex(name, "", buf, buf_size, SCALE(10), SCALE(5), draw->get_clr(clr->white), false, ImRect(total.Min, total.Max - SCALE(25, 0)), &animstate->active, 0, 0, 0, user_clicked);
...
bool c_widgets::games_search_field(std::string name, std::string icon, char* buf, int buf_size) {
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImVec2(gui->content_avail().x, SCALE(40));
    ImRect total = ImRect(pos, pos + size);
    ...
    draw->rect_filled(window->DrawList, total.Min, total.Max, draw->get_clr(clr->widget_layout), SCALE(4));

    if (strlen(buf) == 0) {
        draw->text_clipped(window->DrawList, font->get(inter_medium, 12), total.Min + SCALE(15, 0), total.Max, draw->get_clr(clr->text_inactive), name.data(), gui->text_end(name.data()), 0, { 0, 0.5 });
    }

    draw->text_clipped(window->DrawList, font->get(icons, 12), total.Min, total.Max - SCALE(15, 0), draw->get_clr(clr->accent), icon.data(), 0, 0, { 1.0, 0.5 });

    gui->push_var(style_var_frame_rounding, 0);
    gui->push_font(font->get(inter_medium, 12));

    bool ret = text_field_ex(name, "", buf, buf_size, SCALE(15), SCALE(8), draw->get_clr(clr->white), false, ImRect(total.Min, total.Max - SCALE(40, 0)), &animstate->active, 0, 0, 0, user_clicked);
