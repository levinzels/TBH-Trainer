...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v)
{
    ...
    draw->rect_filled(window->DrawList, bb.Min + SCALE(2, 0), bb.Max - SCALE(2, 0), draw->get_clr(clr->widget_layout), SCALE(100));
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    draw->rect_filled(window->DrawList, grab_rect.Min, grab_rect.Max, draw->get_clr(clr->accent), SCALE(2));
...
bool c_gui::begin(std::string_view name, bool* p_open, window_flags flags)
{
    ...
    if (first_begin_of_the_frame && !window->SkipRefresh)
    {
        ...
        struct anim_t {
            bool wheel = false;
            float next_scroll = 0.f;
        };

        anim_t* t = gui->anim_container(&t, window->ID);

        if (IsKeyDown(ImGuiKey_MouseWheelY)) {
            t->wheel = true;
            t->next_scroll = calc_next_scroll_from_scroll_target_and_clamp(window, false).y;
        }

        window->ScrollTarget = ImVec2(window->ScrollTarget.x, window->ScrollTarget.y + g.NextWindowData.ScrollVal.y);

        if ((window->Scroll.y != t->next_scroll) && t->wheel) {

            if (roundf(window->Scroll.y) == roundf(t->next_scroll)) {
                t->wheel = false;
            }

            gui->easing(window->Scroll.y, t->next_scroll, 17.f, dynamic_easing);
        }
