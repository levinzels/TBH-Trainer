...
void c_gui::set_style()
{
    ...
    style->window_padding = ImVec2(0, 0);
    style->item_spacing = ImVec2(0, 0);
    style->scrollbar_size = SCALE(8.f);
    style->scrollbar_border_padding = SCALE(10, 10);
    style->scrollbar_content_padding = SCALE(0);
}
...
void c_gui::draw_decorations()
{
    ...
    ImDrawList* drawlist = gui->background_drawlist();

    draw->rect_filled(drawlist, pos, pos + size, draw->get_clr(clr->layout), SCALE(elements->window.rounding));
    draw->rect_filled(drawlist, pos, pos + ImVec2(size.x, SCALE(elements->titlebar.height)), draw->get_clr(clr->titlebar_layout), SCALE(elements->window.rounding), draw_flags_round_corners_top);
}
